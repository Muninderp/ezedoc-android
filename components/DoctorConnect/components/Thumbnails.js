...
import { TouchableHighlight, ListView, Image } from "react-native";
...
export default class Thumbnails extends Component {
  constructor(props) {
    ...
    this.state = {
      dataSource: dataSource.cloneWithRows(
        props.streams.filter(stream => stream.id != props.activeStreamId)
      )
    };
  }
  ...
  componentWillReceiveProps(nextProps) {
    let b =
      ...
      nextProps.activeStreamId != this.props.activeStreamId;
    if (b) {
      this.setState({
        dataSource: this.state.dataSource.cloneWithRows(
          nextProps.streams.filter(
            stream => stream.id != nextProps.activeStreamId
          )
        )
      });
    }
